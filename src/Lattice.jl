...
function SIS18_Cell(k1f::Float64, k1d::Float64, k2f::Float64, k2d::Float64)
    # bending magnets
    bendingAngle = 0.2617993878
    rb1 = ThinLens.RBen(2.617993878, bendingAngle, 0, 0)
    rb2 = ThinLens.RBen(2.617993878, bendingAngle, 0, 0)
    ...
    # sextupoles
    ks1c = ThinLens.Sextupole(0.32, k2f, 0)
    ks3c = ThinLens.Sextupole(0.32, k2d, 0)
...
function SIS18_Lattice(k1f::Float64, k1d::Float64, k2f::Float64, k2d::Float64; nested::Bool=true, cellsIdentical::Bool=false)
    if nested
        if cellsIdentical
            cell = SIS18_Cell(k1f, k1d, k2f, k2d)
            return ThinLens.NestedChain([cell for _ in 1:12])
        else
            return ThinLens.NestedChain([SIS18_Cell(k1f, k1d, k2f, k2d) for _ in 1:12])
        end
    else
        if cellsIdentical
            cell = SIS18_Cell(k1f, k1d, k2f, k2d)
            ...
            return ThinLens.FlatChain([SIS18_Cell(k1f, k1d, k2f, k2d) for _ in 1:12])
        end
    end
end
